...
import chess.src.Game.Player;
import chess.src.Game.Square;
...
public class Bishop extends Piece {
    ...
    private boolean isValidPath(Square[][] board, int x1, int y1, int x2, int y2) {
        // base case: bishop reaches the destination squre without any conflicts
        if ((x1 == x2) && (y1 == y2))
            return true;
        ...
        if (board[x1][y1].getPiece() != null) {
            // if theres a piece on the path, it not being on the destiation square or the
    ...
    public boolean isValidMove(Player player, Square[][] board, int x2, int y2) {
        ...
        if (!isValidPath(board, currX, currY, x2, y2)) {
            return false;
        }
